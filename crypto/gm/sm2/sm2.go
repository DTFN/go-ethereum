...
func GenerateKeyFromBigInt(d *big.Int) *PrivateKey {
	c := P256Sm2()
	priv := new(PrivateKey)
	priv.PublicKey.Curve = c
	priv.D = d
...
func NewPrivateKeyFromString(key string) *PrivateKey {
	...
	privKey := GenerateKeyFromBigInt(&keyInt)

	newPrivKey.PublicKey = privKey.PublicKey
	newPrivKey.X = privKey.X
	newPrivKey.Y = privKey.Y
	newPrivKey.D = privKey.D
