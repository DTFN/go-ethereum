...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(1)
	h.txsCh = make(chan core.TxPreEvent, txChanSize)
	h.txsSub = h.txpool.SubscribeTxPreEvent(h.txsCh)
...
func (h *handler) txBroadcastLoop() {
	...
	for {
		select {
		case event := <-h.txsCh:
			h.BroadcastTransactions(types.Transactions{event.Tx}, true)  // First propagate transactions to peers
			h.BroadcastTransactions(types.Transactions{event.Tx}, false) // Only then announce to the rest
